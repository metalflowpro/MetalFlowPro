  ...
  return (
    <div className="space-y-2">
      ...
      <svg viewBox={`0 0 ${W} ${H}`} style={{ width: '100%', maxWidth: W }} className="rounded-lg bg-[#0B1017] border border-mf-border">
        ...
        <text x={W - PAD} y={y(czTop) - 4} fill="#7F8DA3" fontSize="9" textAnchor="end">surface {czTop.toFixed(0)} m</text>
        {shells.map((s, idx) => {
        ...
        <text x={PAD} y={y(czBot) + 14} fill="#7F8DA3" fontSize="9">fond {czBot.toFixed(0)} m</text>
      </svg>
      <div className="flex items-center gap-2 flex-wrap text-[9px] mf-txt4">
        {shells.map((s, idx) => (
          <span key={s.revenueFactor} className="flex items-center gap-1">
            ...
            ×{s.revenueFactor.toFixed(2)}{Math.abs(s.revenueFactor - 1) < 1e-9 ? ' (base)' : ''}
          </span>
        ))}
      </div>
    </div>
