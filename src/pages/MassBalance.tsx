...
function EditCell({
  value, onSave, fmt = (v: number) => v.toFixed(2),
              ...
              <>
                ...
                <div className="grid grid-cols-3 gap-4">
                  ...
                  <div className="card">
                    ...
                    {[
                      ['NaCN total',   `${totalCN.toFixed(2)} kg/h`],
                      ['Chaux (CaO)',  `${totalLime.toFixed(2)} kg/h`],
                      ['Conso. NaCN',  `${(totalCN / project.target_tph).toFixed(3)} kg/t`],
                      ['Conso. CaO',   `${(totalLime / project.target_tph).toFixed(3)} kg/t`],
                      ['Énergie spec.', `${(totalEnergy / project.target_tph).toFixed(1)} kWh/t`],
                    ].map(([k, v]) => <div key={k as string} className="stat-row"><span className="stat-key">{k}</span><span className="stat-val">{v}</span></div>)}
                  </div>
                  <div className="card">
                ...
                <div className="card">
                  ...
                  {(() => {
                    ...
                    return [
                      ...
                      ['Consommation nette',      `${(freshWater / project.target_tph).toFixed(2)} m³/t`],
                    ].map(([k, v]) => <div key={k as string} className="stat-row"><span className="stat-key">{k}</span><span className="stat-val">{v}</span></div>);
                  })()}
                </div>
                <div className="card">
                ...
                <div className="grid grid-cols-3 gap-4">
                  {[
                    ...
                    <div key={s.label} className="card">
                      <div className="text-2xl font-bold font-mono mb-1" style={{ color: s.color }}>{s.pct.toFixed(0)}%</div>
                    ...
                    <tbody>
                      {carbon.map(c => (
                        <tr key={c.id} className={c.is_edited ? 'bg-amber-500/5' : 'hover:bg-mf-hover/40'}>
                          ...
                          <td className="text-right font-mono text-xs">
                            <EditCell value={c.emission_factor} onSave={v => updateCarbon(c.id, 'emission_factor', v)} fmt={v => v.toFixed(4)} />
                            <span className="text-mf-txt4 ml-1">{c.ef_unit}</span>
                          </td>
                          <td className="text-right font-bold font-mono text-xs text-emerald-400">{c.tco2e_year.toFixed(1)}</td>
                      ...
                      <tr className="bg-mf-panel font-semibold">
                        ...
                        <td className="text-right font-bold font-mono text-sm text-emerald-400">{totalCO2.toFixed(0)}</td>
                        <td></td>
                      </tr>
                    </tbody>
                  </table>
            ...
            <div className="mt-4 p-3 bg-mf-panel border border-mf-border rounded-lg">
              ...
              {[
                ...
                <div key={f.l} className="flex items-center gap-3 mb-1.5">
                  ...
                  <span className="text-xs font-mono text-amber-400 w-8">{f.sens.toFixed(2)}</span>
                </div>
              ))}
            </div>
          </div>
