        ...
        {tab === 'overview' && (
          <div className="space-y-4">
            ...
            <div className="grid grid-cols-4 gap-3">
              {[
                { label: 'NPV (actualisation)', value: npv != null ? `${npv.toFixed(1)} M$` : '—', color: npv != null && npv > 0 ? 'text-emerald-400' : 'text-red-400', note: discRate ? `@ ${discRate}% taux d'act.` : 'Taux non configuré' },
                { label: 'TRI (IRR)', value: irr != null ? `${irr.toFixed(1)}%` : '—', color: irr != null && irr > 15 ? 'text-emerald-400' : 'text-amber-400', note: lomYears ? `sur ${lomYears} ans` : 'LOM non configuré' },
                { label: 'Délai de retour', value: payback != null ? `${payback.toFixed(1)} ans` : '—', color: payback != null && payback < 5 ? 'text-emerald-400' : 'text-amber-400', note: totalCapex > 0 ? `CAPEX: ${totalCapex.toFixed(1)} M$` : 'CAPEX non saisi' },
                { label: 'AISC estimé', value: aisc != null ? `$${aisc.toFixed(0)}/oz` : '—', color: aisc != null && aisc < goldPrice * 0.6 ? 'text-emerald-400' : 'text-amber-400', note: `vs. prix Au: $${goldPrice}/oz` },
              ].map(k => (
            ...
            <div className="grid grid-cols-3 gap-3">
              {[
                { label: 'Revenus annuels', value: revenueM != null ? `${revenueM.toFixed(1)} M$` : '—', note: annualOz != null ? `${(annualOz / 1000).toFixed(1)} koz/an` : 'Heures/an requis' },
                { label: 'OPEX annuel', value: annualOpexM != null ? `${annualOpexM.toFixed(1)} M$` : '—', note: totalOpex > 0 ? `${totalOpex.toFixed(2)} $/t` : 'OPEX à saisir' },
                { label: 'EBITDA', value: ebitdaM != null ? `${ebitdaM.toFixed(1)} M$` : '—', note: marginPct != null ? `Marge: ${marginPct.toFixed(1)}%` : '' },
              ].map(k => (
                <div key={k.label} className="card-sm">
          ...
          <div className="space-y-3">
            <div className="flex justify-between items-center">
              <div className="text-sm font-semibold mf-txt">
                CAPEX Total: <span className="text-amber-400">{totalCapex.toFixed(2)} M$</span>
              </div>
              <div className="flex gap-2 items-center">
                      ...
                      {capexLines.map(l => (
                        <tr key={l.id} className="border-b border-white/5 hover:bg-white/5">
                          ...
                          <td className="px-3 py-1.5">
                            ...
                            ) : <span className="font-semibold text-amber-300">{l.value_musd.toFixed(2)}</span>}
                          </td>
                          <td className="px-3 py-1.5 mf-txt3">{l.contingency_pct.toFixed(0)}%</td>
                          <td className="px-3 py-1.5 text-amber-400 font-semibold">
                            {(l.value_musd * (1 + l.contingency_pct / 100)).toFixed(2)}
                          </td>
                          <td className="px-3 py-1.5">
                ...
                <div className="grid grid-cols-3 gap-3">
                  {Object.entries(capexByCategory).map(([cat, val]) => (
                    <div key={cat} className="card-sm text-xs">
                      ...
                      <div className="font-semibold text-amber-400">{val.toFixed(2)} M$</div>
                      <div className="text-[9px] mf-txt4">{totalCapex > 0 ? ((val / totalCapex) * 100).toFixed(1) : 0}%</div>
                    </div>
                  ))}
                </div>
              </>
                      ...
                      {[
                        ...
                        return (
                          <tr key={row.label} className="border-b border-white/5 hover:bg-white/4">
                            ...
                            <td className="px-3 py-2 text-right mf-txt3">{annT > 0 ? (row.val/annT).toFixed(2) : 0}</td>
                            <td className="px-3 py-2 text-right mf-txt3">{annOz > 0 ? (row.val/annOz).toFixed(2) : 0}</td>
                            <td className="px-3 py-2 text-right mf-txt3">{pct > 0 ? `${pct.toFixed(1)}%` : '—'}</td>
                          </tr>
                        );
                      })}
                    </tbody>
                      ...
                      <tr className="border-t-2 border-amber-400/40">
                        ...
                        {(() => {
                          ...
                          return (<>
                            ...
                            <td className="px-3 py-2 text-right font-bold text-amber-400">{annOz>0?(grand/annOz).toFixed(2):0}</td>
                  ...
                  <table className="tbl w-full text-xs min-w-[1000px]">
                    ...
                    <tfoot>
                      <tr className="border-t-2 border-amber-400/40">
                        ...
                        <td className="px-2 py-2 text-right font-bold text-amber-400">{powerRows.reduce((s,r)=>s+r.kw_mec,0).toFixed(0)}</td>
                        <td colSpan={4}/>
                  ...
                  <table className="tbl w-full text-xs">
                    ...
                    <tfoot>
                      <tr className="border-t-2 border-amber-400/40">
                        ...
                        <td className="px-2 py-2 text-right font-bold text-amber-400">{(opexInputs.annual_tonnes>0?reagentRows.reduce((s,r)=>s+(r.conso_unit*opexInputs.annual_tonnes*r.cost_unit),0)/opexInputs.annual_tonnes:0).toFixed(2)}</td>
                        <td/>
                      </tr>
                    </tfoot>
                  </table>
                  ...
                  <tbody>
                    {lomRows.map(r => (
                      <tr key={r.yr} className={`border-b border-white/5 hover:bg-white/5 ${r.cumFcf > 0 ? 'bg-emerald-400/2' : ''}`}>
                        ...
                        <td className="px-3 py-1.5">{(r.tonnes / 1000).toFixed(0)}</td>
                        <td className="px-3 py-1.5 text-amber-400">{r.oz_k.toFixed(1)}</td>
                        <td className="px-3 py-1.5 text-emerald-300">{r.revM.toFixed(1)}</td>
                        <td className="px-3 py-1.5 mf-txt2">{r.opM.toFixed(1)}</td>
                        <td className="px-3 py-1.5 mf-txt3">{r.capM.toFixed(1)}</td>
                        <td className={`px-3 py-1.5 font-semibold ${r.fcf >= 0 ? 'text-emerald-400' : 'text-red-400'}`}>{r.fcf.toFixed(1)}</td>
                        <td className={`px-3 py-1.5 font-bold ${r.cumFcf >= 0 ? 'text-emerald-400' : 'text-red-400'}`}>{r.cumFcf.toFixed(1)}</td>
                      </tr>
                    ))}
                  </tbody>
                </table>
                ...
                return (
                  <div key={row.param} className="flex items-center gap-2 text-xs">
                    ...
                    <div className="w-20 text-[10px] mf-txt4 text-right">
                      {row.low.toFixed(0)} / {row.high.toFixed(0)} M$
                    </div>
                  </div>
                );
              })}
                              ...
                              {FISCAL_REGIMES.filter(r => r.group === group).map(reg => {
                                ...
                                return (
                                  ...
                                  >
                                    ...
                                    <td className="px-2 py-1.5 text-right font-mono mf-txt3">{reg.corp_tax_pct.toFixed(1)}%</td>
                                    <td className="px-2 py-1.5 text-right font-mono mf-txt3">{reg.mining_tax_pct > 0 ? `${reg.mining_tax_pct.toFixed(1)}%` : '—'}</td>
                                    <td className="px-2 py-1.5 text-right font-mono text-amber-300">{reg.royalty_pct > 0 ? `${reg.royalty_pct.toFixed(1)}%` : '—'}</td>
                                    <td className="px-2 py-1.5 text-right font-mono mf-txt4">{reg.depletion_pct > 0 ? `${reg.depletion_pct.toFixed(0)}%` : '—'}</td>
                ...
                <div className="space-y-3">
                  <div className="card-sm">
                    ...
                    <div className="space-y-2.5">
                      {[
                        { label: 'Impôt sur les sociétés', val: `${regime.corp_tax_pct.toFixed(1)}%`, color: 'mf-txt2' },
                        { label: 'Taxe minière spécifique', val: regime.mining_tax_pct > 0 ? `${regime.mining_tax_pct.toFixed(1)}%` : 'N/A', color: 'mf-txt2' },
                        { label: 'Redevance sur revenus', val: regime.royalty_pct > 0 ? `${regime.royalty_pct.toFixed(1)}%` : 'N/A', color: 'text-amber-400' },
                        { label: 'Dépréciation accélérée', val: regime.depletion_pct > 0 ? `${regime.depletion_pct.toFixed(0)}%` : 'N/A', color: 'text-emerald-400' },
                      ].map(f => (
                  ...
                  {annRevM > 0 && (
                    <div className="card-sm space-y-2">
                      ...
                      {[
                        ...
                        <div key={f.label} className="flex justify-between text-xs">
                          <span className="mf-txt3">{f.label}</span>
                          <span className={`font-semibold ${f.color}`}>{f.val > 0 ? `−${f.val.toFixed(1)} M$` : '—'}</span>
                        </div>
                      ))}
                      <div className="border-t border-white/10 pt-2 flex justify-between text-xs">
                        <span className="font-semibold mf-txt">Total taxes</span>
                        <span className="font-bold text-red-400">{totalTaxM > 0 ? `−${totalTaxM.toFixed(1)} M$` : '—'}</span>
                      </div>
                      <div className="flex justify-between text-xs">
                        <span className="mf-txt3">Cash-flow net après taxes</span>
                        <span className={`font-bold ${netCashM > 0 ? 'text-emerald-400' : 'text-red-400'}`}>{netCashM.toFixed(1)} M$</span>
                      </div>
                      {aiscFiscal != null && (
                        <div className="flex justify-between text-xs border-t border-white/10 pt-2">
                          ...
                          <span className="font-bold text-amber-400">${aiscFiscal.toFixed(0)}/oz</span>
                        </div>
                      )}
                    </div>
                  )}
